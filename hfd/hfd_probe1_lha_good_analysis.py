"""
HFD Probe-1 LHA-LHA Analysis -- Good Units Only, Depth 0-345 um
Fed-HFD sessions: 17-22
Includes depth fallback: computes depth from templates.npy + channel_positions.npy
when cluster_info.tsv is missing.
...
def compute_cluster_depths(sorted_path_obj):
    ...
    Uses templates.npy and channel_positions.npy to find peak channel depth.
    Returns dict: {cluster_id: depth}.
    """
    ...
    # For each template, find the channel with max absolute amplitude
    peak_channels = np.argmax(np.max(np.abs(templates), axis=1), axis=1)
    depths = channel_positions[peak_channels, 1]

    # Map spike clusters to templates for robust cluster->depth mapping
    ...
    if spike_clusters_path.exists() and spike_templates_path.exists():
        ...
        # Fallback: assume cluster_id == template_id
...
print("=" * 70)
print("HFD PROBE-1 LHA-LHA ANALYSIS -- GOOD UNITS ONLY (0-345 um)")
...
for group_name, session_list in session_groups.items():
    ...
    for session_name, session_num in session_list:
        ...
        print(f"  Session {session_num}: {len(good_lha_ids)} good LHA units (0-345 um)")
    ...
    if group_pairs:
        ...
        out_file = f"data/hfd_lha_good_connectivity_{group_name}.csv"
...
if not all_dfs:
    print("[ERROR] No data to analyze - no sessions had enough good LHA units")
    print("[DONE] HFD Probe-1 LHA analysis ended (no data)")
    exit()
...
session_means.to_csv("data/hfd_lha_good_session_means.csv", index=False)
...
if len(phase_df) > 0:
    ...
    phase_df[save_cols].to_csv("data/hfd_lha_good_stats_phase_comparison.csv", index=False)
...
existing_means_path = Path("data/double_probe_lha_good_session_means.csv")
...
if existing_means_path.exists():
    ...
    state_3way_df[save_cols].to_csv("data/hfd_lha_good_stats_3way_state_comparison.csv", index=False)
else:
    print("  [WARN] No existing session means at data/double_probe_lha_good_session_means.csv")
...
if len(phase_df) > 0:
    ...
    ax.set_title('HFD LHA-LHA (Good Units 0-345um): Exp vs For (Session-Level)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("figures/hfd_lha_good_phase_comparison.png", dpi=150, bbox_inches='tight')
    print("[OK] Saved hfd_lha_good_phase_comparison.png")
    plt.close()
...
if state_comparison_results and existing_means_path.exists():
    ...
    ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Session Mean Cross-Correlation +/- SEM', fontsize=12, fontweight='bold')
    ax.set_title('LHA-LHA (Good Units 0-345um): Fed vs Fasted vs HFD (Session-Level)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("figures/hfd_lha_good_3way_state_comparison.png", dpi=150, bbox_inches='tight')
    print("[OK] Saved hfd_lha_good_3way_state_comparison.png")
    plt.close()
...
if len(phase_df) > 0:
    ...
    ax.set_title('HFD LHA-LHA (0-345um): Exp vs For (Session-Level)', fontsize=12, fontweight='bold', pad=20)
else:
    axes[0].axis('off')
...
if state_comparison_results:
    ...
    for i in range(1,len(tbl)):
        for j in range(5): t[(i,j)].set_facecolor('#ecf0f1' if i%2==0 else 'white')
    ax.set_title('LHA-LHA (0-345um): Fed vs Fasted vs HFD (Session-Level)', fontsize=12, fontweight='bold', pad=20)
else:
    axes[1].axis('off')
    axes[1].text(0.5, 0.5, 'No 3-way comparison data', ha='center', va='center')

plt.tight_layout()
plt.savefig("figures/hfd_lha_good_summary_tables.png", dpi=150, bbox_inches='tight')
print("[OK] Saved hfd_lha_good_summary_tables.png")
plt.close()

print(f"\n[DONE] HFD Probe-1 LHA good-units analysis complete!")
