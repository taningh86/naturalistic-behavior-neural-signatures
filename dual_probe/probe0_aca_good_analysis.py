"""
Probe-0 (ACA) Full Analysis — Good Units Only
Uses 'good' label from cluster_info.tsv / cluster_group.tsv (no custom QC).
...
def get_good_unit_ids(sorted_path_obj):
    ...
    if ci.exists():
        ...
        # Prefer Phy-curated 'group' if populated, else KSLabel
...
def prebin_spike_trains(sorting, unit_ids):
    ...
    # Find global time range for this session
    all_max = 0
    all_min = np.inf
    spike_trains = {}
    for uid in unit_ids:
...
print("=" * 70)
print("PROBE-0 (ACA) FULL ANALYSIS — GOOD UNITS ONLY")
print("=" * 70)
...
for group_name, session_list in session_groups.items():
    ...
    if group_pairs:
        ...
        out_file = f"data/aca_good_connectivity_{group_name}.csv"
...
# Map session numbers to state/phase
session_meta = {
...
session_means.to_csv("data/double_probe_aca_good_session_means.csv", index=False)
...
if len(state_df) > 0:
    ...
    state_df[save_cols].to_csv("data/double_probe_aca_good_stats_state_comparison.csv", index=False)
...
if len(phase_df) > 0:
    ...
    phase_df[save_cols].to_csv("data/double_probe_aca_good_stats_phase_comparison.csv", index=False)
...
if len(state_df) > 0:
    ...
    ax.set_title('Double Probe ACA-ACA (Good Units): Fed vs Fasted (Session-Level)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("figures/double_probe_aca_good_state_comparison.png", dpi=150, bbox_inches='tight')
    print("[OK] Saved double_probe_aca_good_state_comparison.png")
    plt.close()
...
if len(phase_df) > 0:
    ...
    ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Session Mean Cross-Correlation +/- SEM', fontsize=12, fontweight='bold')
    ax.set_title('Double Probe ACA-ACA (Good Units): Exploration vs Foraging (Session-Level)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    for i, row in phase_df.iterrows():
        ...
        ax.text(i, yp + margin*0.55, f"d = {row['Cohens_d']:.3f}", ha='center', fontsize=8, style='italic')
    plt.tight_layout()
    plt.savefig("figures/double_probe_aca_good_phase_comparison.png", dpi=150, bbox_inches='tight')
    print("[OK] Saved double_probe_aca_good_phase_comparison.png")
    plt.close()
...
if len(state_df) > 0 and len(phase_df) > 0:
    ...
    ax.set_title('ACA-ACA (Good): Fed vs Fasted (Session-Level)', fontsize=12, fontweight='bold', pad=20)
    ...
    for i in range(1,len(tbl)):
        for j in range(5): t[(i,j)].set_facecolor('#ecf0f1' if i%2==0 else 'white')
    ax.set_title('ACA-ACA (Good): Exp vs For (Session-Level)', fontsize=12, fontweight='bold', pad=20)

    plt.tight_layout()
    plt.savefig("figures/double_probe_aca_good_summary_tables.png", dpi=150, bbox_inches='tight')
    print("[OK] Saved double_probe_aca_good_summary_tables.png")
    plt.close()

print(f"\n[DONE] Probe-0 ACA good-units session-level analysis complete!")
