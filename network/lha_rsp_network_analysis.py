"""
LHA-RSP Cross-Regional Network Analysis: Cross-correlation with statistical testing
...
# Filter for Mouse01 Coordinates-1, good units (LHA and RSP)
...
def compute_session_lha_rsp_connectivity(session_key, sorted_path, session_lha_units, session_rsp_units, session_num):
    """Compute LHA-RSP cross-regional connectivity for a single session."""
    try:
        sorting = se.read_kilosort(Path(sorted_path))
        lha_ids = session_lha_units['unit_id'].values
        rsp_ids = session_rsp_units['unit_id'].values

        print(f"\n  Session {session_num}: Computing {len(lha_ids)} LHA × {len(rsp_ids)} RSP pairs...")
        session_start = time.time()

        connectivity_data = []
        pair_count = 0
        total_pairs = len(lha_ids) * len(rsp_ids)

        for i, lha_uid in enumerate(lha_ids):
            spike_times_lha = sorting.get_unit_spike_train(lha_uid)

            for j, rsp_uid in enumerate(rsp_ids):
                spike_times_rsp = sorting.get_unit_spike_train(rsp_uid)
                cc_results = compute_cross_correlation_at_lags(spike_times_lha, spike_times_rsp)
...
for group_name, session_list in session_groups.items():
    ...
    for session_name, session_num in session_list:
        ...
        session_rsp = good_rsp_units[good_rsp_units['session'] == session_key]

        if len(session_lha) == 0 or len(session_rsp) == 0:
            print(f"\n  Session {session_num}: Missing LHA or RSP units")
            continue
        ...
        conn_data = compute_session_lha_rsp_connectivity(session_key, sorted_path, session_lha, session_rsp, session_num)
    ...
    if group_connectivity:
        ...
        print(f"\n  {group_name}: {len(group_connectivity)} LHA-RSP pairs")
...
for group_name, df in all_results.items():
    detail_file = Path(f"data/lha_rsp_connectivity_{group_name}.csv")
...
for group in ['fed_exploration', 'fed_foraging', 'fasted_exploration', 'fasted_foraging']:
    df = pd.read_csv(f"data/lha_rsp_connectivity_{group}.csv")
...
print(f"Total LHA-RSP pairs analyzed: {len(full_df)}")
...
state_df.to_csv("data/lha_rsp_stats_state_comparison.csv", index=False)
print("[OK] lha_rsp_stats_state_comparison.csv")

phase_df.to_csv("data/lha_rsp_stats_phase_comparison.csv", index=False)
print("[OK] lha_rsp_stats_phase_comparison.csv")

lag_df.to_csv("data/lha_rsp_stats_lag_comparison.csv", index=False)
print("[OK] lha_rsp_stats_lag_comparison.csv\n")
...
print("="*70)
print("LHA-RSP CROSS-REGIONAL NETWORK SUMMARY")
print("="*70)
...
print(f"{'='*70}")
print(f"\n[DONE] LHA-RSP cross-regional network analysis complete!")
