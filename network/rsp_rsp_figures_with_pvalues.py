"""
Generate RSP-RSP figures with explicit p-values displayed prominently
"""
...
state_df = pd.read_csv("data/rsp_rsp_stats_state_comparison.csv")
phase_df = pd.read_csv("data/rsp_rsp_stats_phase_comparison.csv")
lag_df = pd.read_csv("data/rsp_rsp_stats_lag_comparison.csv")

print("Generating RSP-RSP figures with explicit p-values...\n")
...
ax.set_title('RSP-RSP Network: State Comparison (Fed vs Fasted)', fontsize=14, fontweight='bold')
ax.set_xticks(x_pos)
...
for i, row in state_df.iterrows():
    ...
    y_pos = max(fed_means[i] + fed_cis[i], fasted_means[i] + fasted_cis[i]) + 0.002
    ...
    ax.text(i, y_pos + 0.0025, f"d = {cohens:.3f}", ha='center', fontsize=9, style='italic')

plt.tight_layout()
fig1_file = Path("figures/rsp_rsp_state_comparison_with_pvalues.png")
...
ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
ax.set_ylabel('Mean Cross-Correlation ± 95% CI', fontsize=12, fontweight='bold')
ax.set_title('RSP-RSP Network: Phase Comparison (Exploration vs Foraging)', fontsize=14, fontweight='bold')
ax.set_xticks(x_pos)
...
for i, row in phase_df.iterrows():
    ...
    y_pos = max(exp_means[i] + exp_cis[i], for_means[i] + for_cis[i]) + 0.001
    ...
    ax.text(i, y_pos, pval_str, ha='center', fontsize=11, fontweight='bold',
            bbox=dict(boxstyle='round,pad=0.4', facecolor='yellow', alpha=0.7))
    ax.text(i, y_pos + 0.0015, f"d = {cohens:.3f}", ha='center', fontsize=9, style='italic')

plt.tight_layout()
fig2_file = Path("figures/rsp_rsp_phase_comparison_with_pvalues.png")
...
for i in range(5):
    table1[(0, i)].set_facecolor('#e74c3c')
...
for i in range(5):
    table2[(0, i)].set_facecolor('#f39c12')
...
plt.tight_layout()
fig3_file = Path("figures/rsp_rsp_statistical_summary_tables.png")
...
bars = ax.bar(lag_df['Lag_ms'].values, lag_means, yerr=lag_cis, capsize=10,
              color=['#c0392b', '#a93226', '#922b21'], alpha=0.8, error_kw={'linewidth': 2},
              width=20)

ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
ax.set_ylabel('Mean Cross-Correlation ± 95% CI', fontsize=12, fontweight='bold')
ax.set_title('RSP-RSP Network: Lag Comparison (Kruskal-Wallis Test)',
...
# Add annotations
y_top = max(lag_means + lag_cis) + 0.0005

ax.plot([10, 50], [y_top + 0.001, y_top + 0.001], 'k-', linewidth=2)
ax.text(30, y_top + 0.0015, 'ns', ha='center', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgray', alpha=0.7))

ax.plot([10, 100], [y_top + 0.005, y_top + 0.005], 'k-', linewidth=2)
ax.text(55, y_top + 0.0055, 'p<0.0001***', ha='center', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='yellow', alpha=0.7))

ax.plot([50, 100], [y_top + 0.008, y_top + 0.008], 'k-', linewidth=2)
ax.text(75, y_top + 0.0085, 'p<0.0001***', ha='center', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='yellow', alpha=0.7))

plt.tight_layout()
fig4_file = Path("figures/rsp_rsp_lag_comparison_with_pvalues.png")
plt.savefig(fig4_file, dpi=150, bbox_inches='tight')
print(f"[OK] Saved: {fig4_file}\n")
plt.close()

print("="*70)
print("ALL RSP-RSP FIGURES GENERATED WITH EXPLICIT P-VALUES")
print("="*70)
print("\nFigures created:")
print(f"  1. {fig1_file}")
print(f"  2. {fig2_file}")
