"""
Generate LHA-RSP figures with explicit p-values displayed prominently
"""
...
state_df = pd.read_csv("data/lha_rsp_stats_state_comparison.csv")
phase_df = pd.read_csv("data/lha_rsp_stats_phase_comparison.csv")
lag_df = pd.read_csv("data/lha_rsp_stats_lag_comparison.csv")

print("Generating LHA-RSP figures with explicit p-values...\n")
...
ax.set_title('LHA-RSP Network: State Comparison (Fed vs Fasted)', fontsize=14, fontweight='bold')
ax.set_xticks(x_pos)
...
plt.tight_layout()
fig1_file = Path("figures/lha_rsp_state_comparison_with_pvalues.png")
...
ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
ax.set_ylabel('Mean Cross-Correlation ± 95% CI', fontsize=12, fontweight='bold')
ax.set_title('LHA-RSP Network: Phase Comparison (Exploration vs Foraging)', fontsize=14, fontweight='bold')
ax.set_xticks(x_pos)
...
plt.tight_layout()
fig2_file = Path("figures/lha_rsp_phase_comparison_with_pvalues.png")
...
for i in range(5):
    table1[(0, i)].set_facecolor('#9b59b6')
...
for i in range(5):
    table2[(0, i)].set_facecolor('#16a085')
...
plt.tight_layout()
fig3_file = Path("figures/lha_rsp_statistical_summary_tables.png")
...
bars = ax.bar(lag_df['Lag_ms'].values, lag_means, yerr=lag_cis, capsize=10,
              color=['#8e44ad', '#7d3c98', '#6c3483'], alpha=0.8, error_kw={'linewidth': 2},
              width=20)

ax.set_xlabel('Time Lag (ms)', fontsize=12, fontweight='bold')
ax.set_ylabel('Mean Cross-Correlation ± 95% CI', fontsize=12, fontweight='bold')
ax.set_title('LHA-RSP Network: Lag Comparison (Kruskal-Wallis Test)',
...
plt.tight_layout()
fig4_file = Path("figures/lha_rsp_lag_comparison_with_pvalues.png")
plt.savefig(fig4_file, dpi=150, bbox_inches='tight')
print(f"[OK] Saved: {fig4_file}\n")
plt.close()

print("="*70)
print("ALL LHA-RSP FIGURES GENERATED WITH EXPLICIT P-VALUES")
print("="*70)
print("\nFigures created:")
print(f"  1. {fig1_file}")
print(f"  2. {fig2_file}")
