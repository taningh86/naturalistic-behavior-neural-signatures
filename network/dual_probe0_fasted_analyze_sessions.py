"""
Dual-Probe Probe-0 (ACA) — FASTED sessions analysis.
Load all fasted double-probe sessions for probe-0 (imec0),
compute QC metrics, and label all units as ACA.

Sessions 11-16 (6 fasted sessions); sessions 17-18 have null paths.
"""
...
print(f"\n{'#'*70}")
print(f"DOUBLE PROBE — PROBE-0 (ACA) — FASTED SESSIONS")
print(f"{'#'*70}")

# Only process fasted sessions (session_11 through session_18)
fasted_session_keys = [f"session_{i}" for i in range(11, 19)]

for session_key in fasted_session_keys:
    if session_key not in sessions:
        print(f"\n[SKIP] {session_key}: Not found in config")
        continue
...
if len(all_metrics) > 0:
    ...
    print(f"\n{'='*70}")
    print(f"COMBINED RESULTS: {len(combined_df)} total ACA units (FASTED)")
    print(f"{'='*70}\n")

    print("SUMMARY (ACA — Probe-0 — FASTED):")
    print("-" * 70)
    ...
    output_file = Path("data/double_probe_probe0_fasted_unit_metrics.csv")
    output_file.parent.mkdir(exist_ok=True, parents=True)
    combined_df.to_csv(output_file, index=False)
    print(f"\n[OK] Saved combined metrics to: {output_file}")
    print("\n[DONE] Fasted QC analysis complete!")
else:
