"""
Stage 3 — Step 2: Localize the LHA mean-speed diet-state effect to a subspace.

Mirrors step1 but for LHA speed instead of ACA curvature.

Procedure
---------
For each session (excl. 13/23/24):
  1. Load LHA spike-rate matrix (50 ms bins, smoothed, z-scored).
  2. PCA. Project onto top-K components for K in {2, 3, 5, 10, 'full'} (LHA has
     fewer units so we cap K at min(K, n_units)).
  3. Compute speed = ||delta_proj|| with sigma=3 smoothing (matches Stage 1).
  4. For each rising/falling phase, take mean speed.
...
SPEED_SIGMA = 3
EXCLUDE = {13, 23, 24}
K_LIST = [2, 3, 5, 10]   # plus 'full' added dynamically; LHA tends to be ~10-30 units
PHASE_TYPES = ('rising', 'falling')
N_BOOT = 5000
RNG = np.random.default_rng(20260427)


def load_lha_cached(session):
    f = CACHE / f'session_{session}_lha.npy'
    if f.exists():
        return np.load(f)
    print(f'  S{session}: loading LHA...')
    matrix, _, _ = load_neural(session, 'LHA')
...
def per_session_speed_means(session, phases_df, K_list_with_full):
    matrix = load_lha_cached(session)
    ...
    for K_label in K_list_with_full:
        ...
        sp = compute_speed(proj)  # length T-1
...
def main():
    ...
    for session in sessions:
        try:
            rows, exp = per_session_speed_means(session, phases_df, K_list_with_full)
        ...
        print(f'  S{session}: {len(rows)} phase rows, n_units={rows[0]["n_units"] if rows else 0}, '
              f'top-5 var-explained = {exp[:5].round(3).tolist()}')

    long = pd.DataFrame(all_rows)
    long['state'] = long['session'].map(state_lookup)
    long.to_csv(OUTDIR / 'lha_per_session_subspace_speed.csv', index=False)
    pd.DataFrame(explained_records).to_csv(OUTDIR / 'lha_explained_variance_long.csv', index=False)

    sess_means = long.groupby(['session', 'state', 'K_label'])['mean_speed'].mean().reset_index()
    sess_means.to_csv(OUTDIR / 'lha_session_means_per_K.csv', index=False)

    diff_rows = []
    for K_label in K_list_with_full:
        sub = sess_means[sess_means.K_label == K_label]
        fed = sub[sub.state == 'fed']['mean_speed'].values
        fas = sub[sub.state == 'fasted']['mean_speed'].values
        hfd = sub[sub.state == 'fed-HFD']['mean_speed'].values
    ...
    diff_df.to_csv(OUTDIR / 'lha_state_diff_vs_K.csv', index=False)

    print('\n=== LHA speed: state diff vs K ===')
    ...
    axes[0].set_ylabel('mean speed diff (a - b)')
    fig.suptitle('LHA speed state-diff vs PCA subspace dim. (red star = CI excludes 0)')
    fig.tight_layout()
    fig.savefig(FIGDIR / 'lha_state_diff_vs_K.png', dpi=140)
    plt.close(fig)
    ...
    ax.set_title('LHA cumulative variance per session')
    fig.tight_layout()
    fig.savefig(FIGDIR / 'lha_explained_variance_per_session.png', dpi=140)
    plt.close(fig)
